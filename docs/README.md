...
These guides are owned and released with `axiom-cli`. They describe intended behavior, not a completed distribution. Resolve cross-component normative references through the pinned `axiom-specs` revision; pack-relative references are conveniences inside the distribution only.
